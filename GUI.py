        ...
        def tv1_click(event):
            ...
            if item:
                ...
                if val and isinstance(val, tuple) and len(val) > 0:
                    ...
                    columns = tv1["columns"]
                    # 生成Cypher查詢的字符串
                    exclude_columns = ['Cnt', 'PageRank', 'Label Propagation', 'Degree Centrality', 'WCC']
                    values_str = ', '.join([f'{column}: "{value}"' for column, value in zip(columns, val) if column != 'isNew' and column not in exclude_columns])

                    cypher_query = "MATCH (n:Node { " + values_str + " }) RETURN n"
...
######## BenchMark End ########
        ...
        def Load_Column():
            ...
            if result is None:
                ...
                for record in result:
                    node = record['n']
                    properties = dict(node)
                    header= [attr for attr in properties if not callable(properties[attr]) and not attr.startswith("__") and not attr.startswith("isNew") and not attr.startswith("time")]
            #new columns
            tv1["columns"] = header
        ...
        def GetDataFromNeo4j(header, data_number):
            ...
            for record in result:
                ...
                new_data = [properties[attr] for attr in header if not callable(properties[attr]) and not attr.startswith("__") and not attr.startswith("isNew") and not attr.startswith("time")]
